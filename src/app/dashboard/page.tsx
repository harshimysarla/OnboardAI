...
import { Users, TrendingUp, Clock, AlertTriangle, Activity, HelpCircle, Lightbulb, LayoutDashboard } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const loadEmployeeData = async (empId: string) => {
    ...
    if (emp) {
      ...
      const requestsList = (Array.isArray(reqs) ? reqs : []).filter(r => r.employee_id === empId);

      setEmployees(employeesList);
      setRequests(requestsList);
      setStats(computeStats(employeesList, requestsList, empId));
              ...
              <div className="mt-8 grid gap-6 lg:grid-cols-2">
                ...
                <Card>
                  ...
                  <CardContent>
                    <ResponsiveContainer width="100%" height={250}>
                      <PieChart>
                        <Pie data={riskDistribution} cx="50%" cy="50%" innerRadius={60} outerRadius={90} dataKey="count" nameKey="level" label={(props: PieLabelRenderProps) => (props.name || "") + ": " + props.value}>
